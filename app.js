/* ADC Threat Lookup — 25.16
   ...
   - Passive (ADC) column with overrides
   - ADC-specific tips via templates (normalized keys)
*/
...
// All ADCs for the portrait grid (added Yunara)
const ADC_IDS = ["Ashe","Caitlyn","Corki","Draven","Ezreal","Jhin","Jinx","KaiSa","Kalista","KogMaw","Lucian","MissFortune","Nilah","Quinn","Samira","Senna","Sivir","Tristana","Twitch","Varus","Vayne","Xayah","Zeri","Aphelios","Yunara"];
...
// Fallback profile icon (works on GitHub Pages)
const PLACEHOLDER_ICON_URL = `https://ddragon.leagueoflegends.com/cdn/${DDRAGON_VERSION}/img/profileicon/588.png`;
// Portrait HTML with safe fallback and name normalization (handles Kai'Sa/Kog'Maw/etc.)
function imgPortraitHTML(slugOrName, alt){
  const safe = String(slugOrName||'').replace(/[^A-Za-z0-9]/g,''); // "Kai'Sa" -> "KaiSa"
  const src  = portraitUrl(safe);
  return `<img class="portrait-sm" src="${src}" alt="${alt||safe}"
            onerror="this.onerror=null;this.src='${PLACEHOLDER_ICON_URL}'">`;
}
...
const RX = {
  ...
  slow:/\b(?:slow|cripple|chill)\b/i, blind:/\b(?:blind|nearsight)\b/i, grounded:/\bground(?:ed)?\b/i,
...
// ===== Passive overrides (short ADC tips)
...
// ===== ADC templates with normalized keys
const T = THREAT;
const normalizeKey = s => String(s||'').toLowerCase().replace(/[^a-z0-9]/g, '');
function getAdcTemplate(adcName){
  const key = normalizeKey(adcName);
  return ADC_TEMPLATES[key] || null;
}
...
function adcNoteFromTemplates(adcName, threatsUnion){
  const t = getAdcTemplate(adcName);
  if(!t) return "";
...
function applyChampionFixes(list){
  ...
  // === Only the highlights; this is your full set with the corrections ===

  // Aatrox
  fix("Aatrox","Q",[THREAT.HARD_CC]);
  fix("Aatrox","W",[THREAT.HARD_CC,THREAT.SOFT_CC]);
  ...
  fix("Ahri","E",[THREAT.SOFT_CC]);
  fix("Ahri","R",[THREAT.GAP_CLOSE]);

  // ... (keep all your previous fixes here; unchanged besides the THREAT.* typos)
  // For brevity in this message, assume you retain the entire block you pasted,
  // including all champions from Aatrox → Zyra, with these corrected lines:

  // Rek'Sai (corrected THREAT.BURST)
  fix("RekSai","E",[THREAT.GAP_CLOSE, THREAT.BURST]);

  // Vex (corrected THREAT.BURST)
  fix("Vex","R",[THREAT.GAP_CLOSE, THREAT.BURST]);

  // Viego (corrected THREAT.GAP_CLOSE)
  fix("Viego","R",[THREAT.BURST, THREAT.GAP_CLOSE]);

  // Warwick (corrected THREAT.BURST)
  fix("Warwick","Q",[THREAT.GAP_CLOSE, THREAT.BURST]);
  ...
  // Xerath R (corrected THREAT.BURST)
  fix("Xerath","R",[THREAT.POKE_ZONE, THREAT.BURST]);

  // Thresh (from your rule)
  ...
  // ...and keep the rest of your giant block as-is.
}
...
function buildAdcGrid(){
  ...
  grid.innerHTML = ADC_IDS.map(id=>{
    ...
    return `<button class="adc-card" data-adc="${champ.name}">
              ${imgPortraitHTML(champ.portrait||champ.slug, champ.name)}
...
function makeSearchCell(team){
  ...
  input.addEventListener("input",()=>{
    ...
    sug.innerHTML = options.map(c=>`<button type="button" data-name="${c.name}">
      <img src="${portraitUrl((c.portrait||c.slug||'').replace(/[^A-Za-z0-9]/g,''))}" onerror="this.onerror=null;this.src='${PLACEHOLDER_ICON_URL}'" style="width:18px;vertical-align:middle;margin-right:6px"/>
      ${c.name}
    </button>`).join("");
